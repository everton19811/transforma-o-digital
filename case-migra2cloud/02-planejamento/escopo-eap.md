...
```mermaid
graph TD
    A[Migração de Plataforma Obsoleta de Planejamento para Google Cloud] --> B[1. Governança]
    A --> C[2. Discovery/AS-IS]
    A --> D[3. Desenho TO-BE]
    A --> E[4. Implantação]
    A --> F[5. Integrações]
    A --> G[6. Gestão de Mudança]
    A --> H[7. Go-live & Sustentação]
```
